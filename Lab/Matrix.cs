...
namespace Lab
{
    internal class Matrix
    {
        ...
        private void CheckIndexes(int row, int column) //Проверка индексов массива
        {
            if (row < 0 || row >= array.GetLength(0)) //Проверка первого индекса (строка)
            {
                throw new IndexOutOfRangeException(nameof(row));
            }
            if (column < 0 || column >= array.GetLength(1)) //Проверка второго индекса (столбец)
            {
